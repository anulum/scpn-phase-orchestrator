...
impl std::fmt::Debug for SimplicialStepper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SimplicialStepper").field("n", &self.n).finish_non_exhaustive()
    }
}
...
impl SimplicialStepper {
    ...
    pub fn step(&mut self, phases: &mut [f64], omegas: &[f64], knm: &[f64], alpha: &[f64], zeta: f64, psi: f64, sigma2: f64) -> SpoResult<()> {
    ...
    pub fn run(&mut self, phases: &mut [f64], omegas: &[f64], knm: &[f64], alpha: &[f64], zeta: f64, psi: f64, sigma2: f64, n_steps: usize) -> SpoResult<()> {
        let alpha_zero = alpha.iter().all(|&a| a == 0.0);
        for _ in 0..n_steps {
            self.compute_derivative(phases, omegas, knm, alpha, zeta, psi, sigma2, alpha_zero);
            for i in 0..self.n {
    ...
    fn compute_derivative(&mut self, theta: &[f64], omegas: &[f64], knm: &[f64], alpha: &[f64], zeta: f64, psi: f64, sigma2: f64, alpha_zero: bool) {
        let n = self.n;
        for i in 0..n {
        ...
        let inv_n2 = if n > 0 { sigma2 / (n as f64 * n as f64) } else { 0.0 };
        ...
        self.deriv_buf.par_iter_mut().enumerate().for_each(|(i, val)| {
            ...
            *val = omegas[i] + pw;
            ...
            if zeta != 0.0 { *val += zs_psi * ci - zc_psi * si; }
        });
    }
}

pub fn simplicial_run(phases: &[f64], omegas: &[f64], knm: &[f64], alpha: &[f64], zeta: f64, psi: f64, sigma2: f64, dt: f64, n_steps: usize) -> Vec<f64> {
    let n = phases.len();
    let mut s = SimplicialStepper::new(n, IntegrationConfig { dt, ..Default::default() }).unwrap();
    let mut p = phases.to_vec();
    s.run(&mut p, omegas, knm, alpha, zeta, psi, sigma2, n_steps).unwrap();
    p
}
