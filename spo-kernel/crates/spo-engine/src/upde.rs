...
impl UPDEStepper {
    ...
    pub fn step(
        ...
        if let Some(ref plast) = self.plasticity {
            plast.update(&self.sin_theta, &self.cos_theta, knm, self.modulator, self.dt);
        }
        wrap_phases(phases);
        Ok(())
    }
    ...
    fn rk4_step(&mut self, phases: &mut [f64], omegas: &[f64], knm: &mut [f64], zeta: f64, psi: f64, alpha: &[f64], dt: f64, alpha_zero: bool) {
        ...
        compute_derivative(n, &self.tmp_phases, &mut self.sin_theta, &mut self.cos_theta, omegas, knm, zeta, psi, alpha, alpha_zero, &mut self.k3);
        for i in 0..n { self.tmp_phases[i] = phases[i] + dt * self.k3[i]; }
        compute_derivative(n, &self.tmp_phases, &mut self.sin_theta, &mut self.cos_theta, omegas, knm, zeta, psi, alpha, alpha_zero, &mut self.k4);
        let dt6 = dt / 6.0;
    ...
    fn rk45_step(&mut self, phases: &mut [f64], omegas: &[f64], knm: &mut [f64], zeta: f64, psi: f64, alpha: &[f64], alpha_zero: bool) {
        ...
        for _ in 0..=3 {
            ...
            compute_derivative(n, &self.tmp_phases, &mut self.sin_theta, &mut self.cos_theta, omegas, knm, zeta, psi, alpha, alpha_zero, &mut self.k5);
            ...
            compute_derivative(n, &self.tmp_phases, &mut self.sin_theta, &mut self.cos_theta, omegas, knm, zeta, psi, alpha, alpha_zero, &mut self.k6);
...
fn compute_derivative(n: usize, theta: &[f64], sin_theta: &mut [f64], cos_theta: &mut [f64], omegas: &[f64], knm: &mut [f64], zeta: f64, psi: f64, alpha: &[f64], alpha_zero: bool, out: &mut [f64]) {
    for i in 0..n {
        ...
        out.par_chunks_mut(block_size).enumerate().for_each(|(block_idx, chunk)| {
            ...
            if alpha_zero {
                for (local_i, val) in chunk.iter_mut().enumerate() {
                    ...
                    let mut fs = s_acc0 + s_acc1 + s_acc2 + s_acc3 + s_acc4 + s_acc5 + s_acc6 + s_acc7;
                    let mut fc = c_acc0 + c_acc1 + c_acc2 + c_acc3 + c_acc4 + c_acc5 + c_acc6 + c_acc7;
...
mod tests {
    ...
    fn make_stepper(n: usize) -> UPDEStepper { UPDEStepper::new(n, IntegrationConfig::default()).unwrap() }
    fn zero_alpha(n: usize) -> Vec<f64> { vec![0.0; n * n] }
    #[test]
    fn zero_n_rejected() { assert!(UPDEStepper::new(0, IntegrationConfig::default()).is_err()); }
    #[test]
    fn single_euler_step() {
        let n = 4; let mut s = make_stepper(n);
        let mut phases = vec![0.0; n]; let omegas = vec![1.0; n];
        let mut knm = vec![0.0; n * n]; let alpha = zero_alpha(n);
        s.step(&mut phases, &omegas, &mut knm, 0.0, 0.0, &alpha).unwrap();
        for &p in &phases { assert!((p - 0.01).abs() < 1e-12); }
    }
    #[test]
    fn synchronisation_tendency() {
        let n = 8; let mut s = make_stepper(n);
        ...
        s.run(&mut phases, &omegas, &mut knm, 0.0, 0.0, &alpha, 1000).unwrap();
