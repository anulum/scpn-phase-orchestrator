...
pub struct SwarmalatorStepper {
    n: usize, dim: usize, dt: f64,
    sin_theta: Vec<f64>, cos_theta: Vec<f64>,
    dx_buf: Vec<f64>, dtheta_buf: Vec<f64>,
}

impl std::fmt::Debug for SwarmalatorStepper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SwarmalatorStepper").field("n", &self.n).field("dim", &self.dim).finish_non_exhaustive()
    }
}
...
impl SwarmalatorStepper {
    pub fn new(n: usize, dim: usize, config: IntegrationConfig) -> SpoResult<Self> {
        ...
        Ok(Self {
            ...
            dx_buf: vec![0.0; n * dim], dtheta_buf: vec![0.0; n],
        })
    }

    pub fn step(&mut self, pos: &mut [f64], phases: &mut [f64], omegas: &[f64], a: f64, b: f64, j: f64, k: f64) -> SpoResult<()> {
        let n = self.n; let dim = self.dim; let dt = self.dt;
        let inv_n = 1.0 / n as f64; let eps = 1e-6;
        ...
        dx_chunks.zip(dt_iter).enumerate().for_each(|(i, (dx_i, dt_i))| {
        ...
        for i in 0..n {
            for d in 0..dim { pos[i * dim + d] += dt * self.dx_buf[i * dim + d] * inv_n; }
            phases[i] = (phases[i] + dt * self.dtheta_buf[i]).rem_euclid(TAU);
        }
        Ok(())
    }

    pub fn run(&mut self, pos: &mut [f64], phases: &mut [f64], omegas: &[f64], a: f64, b: f64, j: f64, k: f64, n_steps: usize) -> (Vec<f64>, Vec<f64>) {
        let n = self.n; let dim = self.dim;
        let mut pt = Vec::with_capacity(n_steps * n * dim);
        let mut pht = Vec::with_capacity(n_steps * n);
        for _ in 0..n_steps {
            self.step(pos, phases, omegas, a, b, j, k).unwrap();
            pt.extend_from_slice(pos); pht.extend_from_slice(phases);
        }
        (pt, pht)
    }
...
pub fn swarmalator_run(pos_init: &[f64], phases_init: &[f64], omegas: &[f64], n: usize, dim: usize, dt: f64, a: f64, b: f64, j: f64, k: f64, n_steps: usize) -> (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>) {
    let mut s = SwarmalatorStepper::new(n, dim, IntegrationConfig { dt, ..Default::default() }).unwrap();
    let mut p = pos_init.to_vec(); let mut ph = phases_init.to_vec();
