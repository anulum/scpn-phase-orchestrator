...
impl StuartLandauStepper {
    ...
    pub fn step(
        ...
        match self.method {
            Method::RK45 => {
                self.rk45_step(
                    state, omegas, mu, knm, knm_r, zeta, psi, alpha, epsilon, alpha_zero,
                );
            }
            _ => {
...
fn compute_derivative(
    ...
    if n >= 256 {
        phase_out
            .par_iter_mut()
            .zip(amp_out.par_iter_mut())
            .enumerate()
            .for_each(|(i, (p_val, a_val))| {
                let offset = i * n;
                ...
                if alpha_zero {
                    ...
                    for j in 0..n {
                        ...
                        a_fs +=
                            knm_r[offset + j] * r[j].max(0.0) * (diff - alpha[offset + j]).cos();
                    }
                }
                *p_val = omegas[i] + p_fs;
                if zeta != 0.0 {
                    *p_val += zs_psi * ct[i] - zc_psi * st[i];
                }
                let ri = r[i];
        ...
        for i in 0..n {
            ...
            if alpha_zero {
                for j in 0..n {
                    ...
                    a_fs += knm_r[offset + j]
                        * r[j].max(0.0)
                        * (cos_theta[j] * cos_theta[i] + sin_theta[j] * sin_theta[i]);
                }
            } else {
                for j in 0..n {
                    let diff = theta[j] - theta[i];
            ...
            if zeta != 0.0 {
                phase_out[i] += zs_psi * cos_theta[i] - zc_psi * sin_theta[i];
            }
            let ri = r[i];
