...
impl std::fmt::Debug for InertialStepper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("InertialStepper")
            .field("n", &self.n)
...
impl InertialStepper {
    pub fn new(n: usize, config: IntegrationConfig) -> SpoResult<Self> {
        ...
        Ok(Self {
            ...
            tmp_th: vec![0.0; dim],
            tmp_od: vec![0.0; dim],
            sin_theta: vec![0.0; dim],
            cos_theta: vec![0.0; dim],
        })
    }
    ...
    pub fn step(
        ...
        for i in 0..n {
            let raw = theta[i]
                + dt6 * (self.k1t[i] + 2.0 * self.k2t[i] + 2.0 * self.k3t[i] + self.k4t[i]);
            theta[i] = raw.rem_euclid(TAU);
            omega_dot[i] +=
                dt6 * (self.k1o[i] + 2.0 * self.k2o[i] + 2.0 * self.k3o[i] + self.k4o[i]);
        }
        Ok(())
    }
}
...
pub fn inertial_step(
    ...
    s.step(&mut th, &mut od, power, knm, inertia, damping)
        .expect("stepper init failed");
    (th, od)
}
