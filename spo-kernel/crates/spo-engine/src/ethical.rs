...
pub fn compute_ethical_cost(
    ...
    let (phi_ethics, n_violated) = compute_cbf_penalties(
        r, lam2, knm, kappa, r_min, connectivity_min, max_coupling,
    );
...
fn compute_sec_inputs(phases: &[f64], knm: &[f64], n: usize) -> (f64, f64, f64, f64) {
    ...
    let q = if n_possible > 0 { n_nonzero as f64 / n_possible as f64 } else { 0.0 };
...
fn compute_cbf_penalties(
    r: f64, lam2: f64, knm: &[f64], kappa: f64,
...
mod tests {
    ...
    fn test_empty_phases() {
        let (j, phi, c15, n_v) = compute_ethical_cost(
            &[], &[], 0, 0.4, 0.3, 0.2, 0.1, 1.0, 0.2, 0.1, 5.0,
        );
        assert_eq!(j, 0.0);
        assert_eq!(phi, 0.0);
        assert_eq!(c15, 1.0);
    ...
    fn test_synchronised_high_coupling() {
        ...
        let (j, _, c15, _) = compute_ethical_cost(
            &phases, &knm, n, 0.4, 0.3, 0.2, 0.1, 1.0, 0.2, 0.1, 5.0,
        );
    ...
    fn test_no_coupling_violation() {
        ...
        let (_, _, _, n_v) = compute_ethical_cost(
            &phases, &knm, n, 0.4, 0.3, 0.2, 0.1, 1.0, 0.5, 0.1, 5.0,
        );
    ...
    fn test_high_coupling_violation() {
        ...
        let (_, phi, _, n_v) = compute_ethical_cost(
            &phases, &knm, n, 0.4, 0.3, 0.2, 0.1, 1.0, 0.0, 0.0, 5.0,
        );
    ...
    fn test_c15_decomposition() {
        ...
        let (j, phi, c15, _) = compute_ethical_cost(
            &phases, &knm, n, 0.4, 0.3, 0.2, 0.1, 1.0, 0.2, 0.1, 5.0,
        );
        assert!(
