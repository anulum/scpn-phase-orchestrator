...
func rk45Step(
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		computeDerivative(k4, tmp, omegas, knm, alpha, zeta, psi, n)
		for i := 0; i < n; i++ {
			tmp[i] = phases[i] + dt*(dpA51*k1[i]+dpA52*k2[i]+dpA53*k3[i]+dpA54*k4[i])
		}
		computeDerivative(k5, tmp, omegas, knm, alpha, zeta, psi, n)
		for i := 0; i < n; i++ {
			tmp[i] = phases[i] + dt*(dpA61*k1[i]+dpA62*k2[i]+dpA63*k3[i]+dpA64*k4[i]+dpA65*k5[i])
		}
		computeDerivative(k6, tmp, omegas, knm, alpha, zeta, psi, n)
		for i := 0; i < n; i++ {
			y5[i] = phases[i] + dt*(dpB5_0*k1[i]+dpB5_2*k3[i]+dpB5_3*k4[i]+dpB5_4*k5[i]+dpB5_5*k6[i])
		}
		computeDerivative(k7, y5, omegas, knm, alpha, zeta, psi, n)
		errNorm := 0.0
		for i := 0; i < n; i++ {
			y4 := phases[i] + dt*(dpB4_0*k1[i]+dpB4_2*k3[i]+dpB4_3*k4[i]+
...
func upderRunOmegaSchedule(
	phases []float64,
	omegaSchedule, knm, alpha []float64,
	zeta, psi, dt float64,
	nSteps, method int,
	nSubsteps int,
	atol, rtol float64,
	...
	for step := 0; step < nSteps; step++ {
		omegas := omegaSchedule[step*n : (step+1)*n]
		switch method {
		case 2: // rk45
			lastDt = rk45Step(
				phases, omegas, knm, alpha,
...
// UPDERunOmegaSchedule integrates with a row-major frequency schedule:
// omegaSchedule[step*n + oscillator].
//
//export UPDERunOmegaSchedule
func UPDERunOmegaSchedule(
	phasesPtr *C.double,
