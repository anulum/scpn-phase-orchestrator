...
- `spo digital-twin-observability-bundle` now bundles only scheduler records
  ...
  edited row under a stale hash was bundled as if it were the sealed record.
...
- Refresh the fail-closed `rustup-init` checksum to the official `1.29.1`
  release digest so the release container can be built, scanned, attested,
  signed, and published after the upstream installer changed. Python and Rust
  reactor-semantics APIs are unchanged from `1.4.0`.
...
- A byte-canonical L1 FRC-compression-MIF physical-payload request binds the
  ...
  magneto-inertial candidates, and names twelve missing physical-evidence
  obligations. Its sealed request identity is linked from the producer-
  evidence priority register, while schema allocation, observation, phase,
  CONTROL, execution, and actuation remain fail closed.
...
- A reactor-family-neutral, byte-canonical and digest-sealed device diagnostic-
  ...
  focus, theta-pinch, Z-pinch, MagLIF, mechanical-or-liquid-liner MIF,
  plasma-jet MIF, laser-ICF, ion/electron-beam ICF, and projectile-impact ICF
  fixtures exercise distinct event-cycle, complex-mode, bounded-feature, and
  numerical-phase declarations.
  Their typed signal and clock rows remain synthetic design declarations and
...
- Cross-dataset generalisation evaluations of the certified grid modal-growth
  ...
  (`bench.early_warning_leadtime_isone`,
  `examples/real_data/iso_ne_forced_oscillation/`) and a 13-case WECC 240-bus
  statistical leg (`bench.early_warning_leadtime_wecc`,
  `examples/real_data/wecc_240_osl/`). Both report the frozen-transfer and
