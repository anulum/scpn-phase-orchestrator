...
class TestSynapseChannelBridgeAsync:
    ...
    @pytest.mark.asyncio
    async def test_listen_once_rejects_duplicate_json_object_keys(
        self,
        caplog: pytest.LogCaptureFixture,
    ) -> None:
        bridge = self._make_bridge()
...
# Pipeline wiring: SynapseChannelBridge connect/listen/send tests above prove
# the async WebSocket bridge is functional for inter-agent communication.
