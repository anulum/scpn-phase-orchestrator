...
def test_epidemic_sir_value_alignment_blocks_excessive_npi_drive():
    spec = load_binding_spec(DOMAINPACKS_DIR / "epidemic_sir" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.05,
        ttl_s=0.5,
        justification="review candidate exceeds npi-drive prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-npi-drive-step"
    assert decision.actions_to_apply[0].justification == (
        "epidemic value guard npi hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_epidemic_sir_value_alignment_allows_bounded_vaccination_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "epidemic_sir" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    action = ControlAction(
        ...
        value=0.75,
        ttl_s=0.5,
        justification="bounded vaccination coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_agent_coordination_value_alignment_blocks_excessive_deadline_drive():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "agent_coordination" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=0.8,
        ttl_s=5.0,
        justification="review candidate exceeds deadline-drive prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-deadline-drive-step"
    assert decision.actions_to_apply[0].justification == (
        "coordination value guard conflict hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_agent_coordination_value_alignment_allows_bounded_task_coupling():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "agent_coordination" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    action = ControlAction(
        ...
        value=0.35,
        ttl_s=5.0,
        justification="bounded task redistribution review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_pll_clock_value_alignment_blocks_excessive_reference_drive():
    spec = load_binding_spec(DOMAINPACKS_DIR / "pll_clock" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=0.9,
        ttl_s=10.0,
        justification="review candidate exceeds reference-drive prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-reference-drive-step"
    assert decision.actions_to_apply[0].justification == (
        "pll value guard holdover-safe reference hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_pll_clock_value_alignment_allows_bounded_loop_bandwidth():
    spec = load_binding_spec(DOMAINPACKS_DIR / "pll_clock" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    action = ControlAction(
        ...
        value=0.45,
        ttl_s=5.0,
        justification="bounded loop bandwidth review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_digital_twin_value_alignment_blocks_excessive_twin_coupling():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "digital_twin_nchannel" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.2,
        ttl_s=1.0,
        justification="review candidate exceeds twin-coupling prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-twin-coupling-step"
    assert decision.actions_to_apply[0].justification == (
        "digital twin value guard residual hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_digital_twin_value_alignment_allows_policy_scale_line_lag_correction():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "digital_twin_nchannel" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    lag_actuator = next(act for act in spec.actuators if act.name == "line_phase_lag")

    assert lag_actuator.limits[0] <= -0.04 <= lag_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="alpha",
        scope="layer_1",
        value=-0.04,
        ttl_s=1.0,
        justification="bounded line lag correction review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_edge_consensus_value_alignment_blocks_excessive_edge_coupling():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "edge_consensus_nchannel" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.1,
        ttl_s=1.0,
        justification="review candidate exceeds edge-coupling prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-edge-coupling-step"
    assert decision.actions_to_apply[0].justification == (
        "edge consensus value guard gateway hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_edge_consensus_value_alignment_allows_policy_scale_gateway_lag():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "edge_consensus_nchannel" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    lag_actuator = next(act for act in spec.actuators if act.name == "gateway_lag")

    assert lag_actuator.limits[0] <= -0.05 <= lag_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="alpha",
        scope="layer_1",
        value=-0.05,
        ttl_s=1.0,
        justification="bounded gateway lag correction review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_rotating_machinery_value_alignment_blocks_excessive_speed_step():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "rotating_machinery" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.3,
        ttl_s=3.0,
        justification="review candidate exceeds speed-setpoint prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-speed-setpoint-step"
    assert decision.actions_to_apply[0].justification == (
        "rotating machinery value guard coast-down hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_rotating_machinery_value_alignment_allows_policy_scale_bearing_stiffness():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "rotating_machinery" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    stiffness_actuator = next(
        act for act in spec.actuators if act.name == "bearing_stiffness"
    )

    assert stiffness_actuator.limits[0] <= 1.5 <= stiffness_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="K",
        scope="global",
        value=1.5,
        ttl_s=5.0,
        justification="bounded bearing-stiffness review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_bio_stub_value_alignment_blocks_excessive_entrainment():
    spec = load_binding_spec(DOMAINPACKS_DIR / "bio_stub" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=0.9,
        ttl_s=10.0,
        justification="review candidate exceeds entrainment prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])
    ...
    assert decision.violations[0].constraint == "limit-entrainment-drive-step"
    assert decision.actions_to_apply[0].justification == (
        "bio stub value guard homeostatic hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_bio_stub_value_alignment_allows_policy_scale_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "bio_stub" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    coupling_actuator = next(
    ...
    assert coupling_actuator.limits[0] <= 0.15 <= coupling_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="K",
        scope="global",
        value=0.15,
        ttl_s=5.0,
        justification="bounded inter-scale coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_vortex_shedding_value_alignment_blocks_excessive_blowing_rate():
    spec = load_binding_spec(DOMAINPACKS_DIR / "vortex_shedding" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.2,
        ttl_s=1.0,
        justification="review candidate exceeds blowing-rate prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-blowing-rate-step"
    assert decision.actions_to_apply[0].justification == (
        "vortex shedding value guard passive-flow hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_vortex_shedding_value_alignment_allows_bounded_wake_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "vortex_shedding" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    coupling_actuator = next(
        act for act in spec.actuators if act.name == "coupling_global"
    ...
    assert coupling_actuator.limits[0] <= 0.4 <= coupling_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="K",
        scope="global",
        value=0.4,
        ttl_s=1.0,
        justification="bounded wake coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_musical_acoustics_value_alignment_blocks_excessive_tempo_drive():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "musical_acoustics" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.4,
        ttl_s=1.0,
        justification="review candidate exceeds tempo-drive prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-tempo-drive-step"
    assert decision.actions_to_apply[0].justification == (
        "musical acoustics value guard rest-tempo hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_musical_acoustics_value_alignment_allows_bounded_harmonic_coupling():
    spec = load_binding_spec(
        DOMAINPACKS_DIR / "musical_acoustics" / "binding_spec.yaml"
    )
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    assert coupling_actuator.limits[0] <= 0.5 <= coupling_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="K",
        scope="global",
        value=0.5,
        ttl_s=1.0,
        justification="bounded harmonic coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_gene_oscillator_value_alignment_blocks_excessive_inducer_dose():
    spec = load_binding_spec(DOMAINPACKS_DIR / "gene_oscillator" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.3,
        ttl_s=1.0,
        justification="review candidate exceeds inducer-dose prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-inducer-dose-step"
    assert decision.actions_to_apply[0].justification == (
        "gene oscillator value guard basal-induction hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_gene_oscillator_value_alignment_allows_bounded_quorum_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "gene_oscillator" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    coupling_actuator = next(
        act for act in spec.actuators if act.name == "coupling_global"
    ...
    action = ControlAction(
        ...
        value=0.4,
        ttl_s=1.0,
        justification="bounded quorum coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_geometry_walk_value_alignment_blocks_excessive_graph_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "geometry_walk" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=1.5,
        ttl_s=1.0,
        justification="review candidate exceeds graph-coupling prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-graph-coupling-step"
    assert decision.actions_to_apply[0].justification == (
        "geometry walk value guard low-coupling hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_geometry_walk_value_alignment_allows_bounded_graph_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "geometry_walk" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    coupling_actuator = next(
        act for act in spec.actuators if act.name == "coupling_global"
    ...
    action = ControlAction(
        ...
        value=0.4,
        ttl_s=1.0,
        justification="bounded graph coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
...
def test_minimal_domain_value_alignment_blocks_excessive_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "minimal_domain" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    ...
    unsafe = ControlAction(
        ...
        value=2.2,
        ttl_s=1.0,
        justification="review candidate exceeds minimal coupling prior",
    )
    decision = ValueAlignmentGuard(policy).evaluate([unsafe])

    assert not decision.satisfied
    assert decision.blocked_actions == (unsafe,)
    assert decision.violations[0].constraint == "limit-minimal-coupling-step"
    assert decision.actions_to_apply[0].justification == (
        "minimal domain value guard baseline-coupling hold"
    )
    assert decision.to_audit_record()["violations"][0]["counterfactual"] == (
        "blocked_action_prevents_constraint_violation"
    )
...
def test_minimal_domain_value_alignment_allows_bounded_coupling():
    spec = load_binding_spec(DOMAINPACKS_DIR / "minimal_domain" / "binding_spec.yaml")
    policy = value_alignment_policy_from_binding_spec(spec)
    coupling_actuator = next(
        act for act in spec.actuators if act.name == "coupling_global"
    ...
    assert coupling_actuator.limits[0] <= 0.6 <= coupling_actuator.limits[1]
    assert policy is not None
    action = ControlAction(
        knob="K",
        scope="global",
        value=0.6,
        ttl_s=1.0,
        justification="bounded minimal coupling review candidate",
    )
    decision = ValueAlignmentGuard(policy).evaluate([action])
