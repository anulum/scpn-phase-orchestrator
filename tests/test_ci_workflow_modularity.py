...
# SCPN Phase Orchestrator — CI Workflow Modularity Tests
  ...
  ci-gate:
    ...
    steps:
      ...
      - env:
        ...
        run: python tools/check_ci_category_results.py
""",
        encoding="utf-8",
    )
...
def test_modularity_audit_reports_policy_size_and_coordinator_failures(
    ...
    coordinator.write_text(
        ...
        .replace("python tools/check_ci_category_results.py", "echo unchecked"),
        encoding="utf-8",
    )
...
def test_category_graph_rejects_unknown_self_and_cyclic_dependencies() -> None:
    """The policy graph must be closed, acyclic, and free of self-dependencies."""
    policy = _policy()
    policy["categories"].append(
        {
            "id": "integration",
            "workflow": ".github/workflows/ci-integration.yml",
            "caller_needs": ["unit-quality", "missing"],
            "jobs": ["integration"],
        }
    )
    policy["categories"][0]["caller_needs"] = ["unit-quality", "integration"]
    errors: list[str] = []

    modularity._check_category_graph(policy, errors)
