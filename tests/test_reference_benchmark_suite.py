...
def test_lyapunov_polyglot_parity_gate_reports_all_language_slots() -> None:
    out = benchmark_lyapunov_polyglot_parity_gate(
        n=3,
        n_steps=40,
        qr_interval=10,
        calls=1,
    ...
    assert out["suite"] == "lyapunov_polyglot_parity_gate"
    ...
    assert out["acceptance_passed"] == 1
    assert len(str(out["reference_spectrum_sha256"])) == 64
    assert len(str(out["benchmark_sha256"])) == 64
    assert float(out["steps_per_second"]) > 0.0
    ...
    for record in records:
        if record["status"] == "available":
            ...
            assert record["spectrum_sha256"] is not None
...
def test_npe_polyglot_parity_gate_reports_all_language_slots() -> None:
    out = benchmark_npe_polyglot_parity_gate(n=8, calls=1, seed=2026)
    records = json.loads(str(out["backend_records_json"]))
    thresholds = json.loads(str(out["acceptance_thresholds_json"]))
    ...
    assert len(str(out["reference_distance_sha256"])) == 64
    assert len(str(out["reference_npe_sha256"])) == 64
    assert len(str(out["benchmark_sha256"])) == 64
    assert float(out["steps_per_second"]) > 0.0
    assert [record["backend"] for record in records] == [
        "rust",
    ...
    for record in records:
        if record["status"] == "available":
            ...
            assert record["ms_per_call"] is not None
            assert record["max_distance_abs_error"] <= record["tolerance"]
            assert record["npe_abs_error"] <= record["tolerance"]
            assert record["max_abs_error"] <= record["tolerance"]
        else:
            assert record["status"] == "unavailable"
            assert record["unavailable_reason"]
...
def test_recurrence_polyglot_parity_gate_reports_all_language_slots() -> None:
    ...
    assert len(str(out["reference_recurrence_sha256"])) == 64
    assert len(str(out["reference_cross_recurrence_sha256"])) == 64
    assert len(str(out["reference_self_cross_sha256"])) == 64
    assert len(str(out["benchmark_sha256"])) == 64
    assert float(out["steps_per_second"]) > 0.0
    assert [record["backend"] for record in records] == [
        "rust",
