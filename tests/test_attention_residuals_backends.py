...
# SCPN Phase Orchestrator — Per-backend parity tests for AttnRes

"""Per-backend parity tests for the AttnRes fallback chain.

Complements ``test_attention_residuals.py`` (which covers the
algorithm invariants through the active backend) by exercising each
non-Python backend individually and comparing its numerical output
against the NumPy reference implementation. Any drift between
backends is a silent physics bug — these tests guard against it.

Each backend is gated on its toolchain being present:

* Rust — always present in a working SPO dev environment (built by
  maturin into ``.venv``). Expect bit-exact parity.
* Mojo — needs ``mojo/attnres_mojo`` compiled on disk. Expect parity
  to ~1e-14 (text round-trip rounding).
* Julia — needs ``juliacall`` installed and ``julia/attnres.jl`` on
  disk. Expect bit-exact parity.
* Go — needs ``go/libattnres.so`` compiled on disk. Expect bit-exact
  parity.
...
def _force_backend(
    backend: str, knm: np.ndarray, theta: np.ndarray, **kw: float | int
) -> np.ndarray:
...
def _python_reference(
    knm: np.ndarray, theta: np.ndarray, **kw: float | int
) -> np.ndarray:
...
class TestRustParity:
    ...
    def test_bit_exact_parity(self, n: int, seed: int) -> None:
        ...
        py = _python_reference(knm, theta, block_size=4, lambda_=0.5)
        rs = _force_backend("rust", knm, theta, block_size=4, lambda_=0.5)
...
class TestJuliaParity:
    ...
    def test_bit_exact_parity(self, n: int) -> None:
        """Julia port should match NumPy to within float64 precision.

        Hypothesis is not used here because juliacall's one-time
        bootstrap is expensive; we use three hand-picked sizes and
        fixed seeds instead."""
        rng = np.random.default_rng(42 + n)
        knm = _symmetric_knm(n, seed=42 + n)
        theta = rng.uniform(0.0, TWO_PI, size=n)
        py = _python_reference(knm, theta, block_size=3, lambda_=0.5)
        jl = _force_backend("julia", knm, theta, block_size=3, lambda_=0.5)
...
class TestGoParity:
    ...
    def test_bit_exact_parity(self, n: int, seed: int) -> None:
        ...
        theta = rng.uniform(0.0, TWO_PI, size=n)
        py = _python_reference(knm, theta, block_size=4, lambda_=0.5)
        go = _force_backend("go", knm, theta, block_size=4, lambda_=0.5)
        # Go's libm should give bit-exact result for this pure-arithmetic
        # kernel; allow atol=1e-12 to absorb any cgo FP env difference.
        np.testing.assert_allclose(go, py, atol=1e-12)

    def test_invalid_block_size_surfaced(self) -> None:
        knm = _symmetric_knm(4, seed=0)
        theta = np.zeros(4)
...
class TestMojoParity:
    ...
    def test_numerical_parity(self, n: int) -> None:
        """Mojo port ships through a text stdin/stdout protocol (Mojo
        0.26 UnsafePointer ABI is not yet stable); expect small
        rounding from the 17-digit float serialisation.
        """
        rng = np.random.default_rng(13 + n)
        knm = _symmetric_knm(n, seed=13 + n)
        theta = rng.uniform(0.0, TWO_PI, size=n)
        py = _python_reference(knm, theta, block_size=3, lambda_=0.5)
        mj = _force_backend("mojo", knm, theta, block_size=3, lambda_=0.5)
        # 17-digit repr round-trip tolerance budget: float64 has
        # ~15–17 decimal digits of precision; allow 1e-13.
...
class TestCrossBackendConsistency:
    ...
    def test_all_backends_agree_on_single_shot(self) -> None:
        """Every loaded backend produces the same output for a single
        fixed (knm, theta) call up to its per-backend tolerance."""
        rng = np.random.default_rng(2026)
        n = 10
        knm = _symmetric_knm(n, seed=2026)
        theta = rng.uniform(0.0, TWO_PI, size=n)

        # Per-backend tolerance budget: Python/Rust/Go/Julia are all
        # pure f64 arithmetic; Mojo incurs text-protocol rounding.
        tolerances = {
            "rust": 1e-12,
            "julia": 1e-12,
            "go": 1e-12,
        ...
        ref = _python_reference(knm, theta, block_size=3, lambda_=0.5)
        for backend in AVAILABLE_BACKENDS:
            out = _force_backend(
                backend, knm, theta, block_size=3, lambda_=0.5
            )
