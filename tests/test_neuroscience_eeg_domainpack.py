...
# SCPN Phase Orchestrator — Neuroscience EEG domainpack tests
...
from domainpacks.neuroscience_eeg import run as eeg_run
from scpn_phase_orchestrator.binding import load_binding_spec, validate_binding_spec

SPEC_PATH = Path("domainpacks/neuroscience_eeg/binding_spec.yaml")
EXPECTED_LAYERS = ["delta", "theta", "alpha", "beta", "gamma", "network"]
...
class TestSpec:
    ...
    def test_name(self, spec) -> None:
        assert spec.name == "neuroscience_eeg"
...
class TestLayerMap:
    def test_layer_map_is_contiguous_and_complete(self, spec) -> None:
        layer_map = eeg_run._build_layer_map(spec)
