...
# SCPN Phase Orchestrator — ICF-impact diagnostic-plan intake tests
"""Exercise exact ICF-impact producer bytes at SPO's design-review boundary."""
...
FIXTURES = Path("tests/fixtures/icf_impact_diagnostic_plan")
SOURCE_REVISION = "397f1f2a5fb2af1ad620174a43720eb1c7b5de5f"
MANIFEST_SHA256 = "587b88857cc7a1d3014e019b7e5e110b6424608e7bdb5fd8c838d9e583d6ce36"
FIXTURE_SHA256 = "33fdafa9716ab7436a245ddba48dee6e9a1ed97397315bd010aed7dff9cd16ff"
ARTIFACT_SHA256 = "4485dffa189017837f8234791b9b160fa12b9da3ab2ea71e9a2128f55f7ca162"
REVIEW_ID = "eeefac32254f871dc94ce655353b60327f2aa1e7dde566bd92c89c86cb8eaa84"
REVIEW_SHA256 = "5035b44a327b916f662125cc452777fb30bc43c6ee37642d354d2c46c2ff60e3"
...
def test_exact_icf_impact_producer_objects_are_digest_pinned() -> None:
    assert (
...
def test_exact_icf_impact_fixture_produces_one_design_review() -> None:
    review = _review()

    assert review.source_project == "SCPN-ICF-IMPACT-CORE"
    ...
    assert review.configurations == ("projectile_or_impact_icf",)
    assert review.plan_identifier == "icf_impact_reference_plan"
    assert review.frame_ids == ("frm_chamber", "frm_launcher")
...
def test_icf_impact_signal_meanings_remain_distinct() -> None:
    meanings = {
...
def test_icf_impact_clocks_do_not_claim_facility_correlation() -> None:
...
def test_icf_impact_review_is_canonical_schema_valid_and_round_trips() -> None:
    review = _review()
...
def test_icf_impact_review_does_not_equate_or_inherit_icf_evidence() -> None:
    review = _review()
    profiles = tuple(
    ...
    assert len({profile.configuration for profile in profiles}) == 1
    assert "scpn_icf_impact_core" not in sys.modules
...
def test_icf_impact_timing_clock_and_carrier_drift_is_refused(
    channel_index: int,
    field: str,
    value: object,
...
def test_icf_impact_tighter_valid_timing_bound_stays_non_evidentiary() -> None:
    manifest, envelope, plan = _records()
    plan["channels"][1]["timing_uncertainty_s"] = 5e-10

    review = _review(manifest=manifest, envelope=envelope, plan=plan)
    signal = next(
        item
        for item in review.signal_reviews
        if item.channel_identifier == "ch_impact_timing_train"
    )
...
@pytest.mark.parametrize(
    "configuration",
    ("laser_icf_direct_drive", "ion_beam_icf", "beam_target"),
)
def test_icf_impact_sibling_configuration_assignment_is_refused(
    configuration: str,
) -> None:
    manifest, envelope, plan = _records()
...
def test_icf_impact_authority_escalation_is_refused() -> None:
    manifest, envelope, plan = _records()
