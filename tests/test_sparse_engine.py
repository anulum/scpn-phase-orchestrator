...
class TestSparseUPDEEngine:
    def test_compare_with_dense(self):
        ...
        p_sparse = engine_sparse.step(phases, omegas, row_ptr, col_indices, knm_values, zeta, psi, alpha_values)
    ...
    def test_run_sparse(self):
        ...
        # Run 100 steps
        p_final = engine.run(phases, omegas, row_ptr, col_indices, knm_values, 0.0, 0.0, alpha_values, 100)
    ...
    def test_sparse_plasticity(self):
        ...
        # Step
        engine.step(phases, omegas, row_ptr, col_indices, knm_values, 0.0, 0.0, alpha_values)
        
        # knm_values[0] (which is knm[0,1]) should have increased
        # delta = lr * mod * cos(th_j - th_i) * dt
        # delta = 1.0 * 1.0 * cos(0.1 - 0.0) * 0.01 = cos(0.1) * 0.01 ≈ 0.995 * 0.01 ≈ 0.00995
