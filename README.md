...
> **Release candidate v1.4.3** — The core UPDE engine, 3-channel oscillator extraction (P/I/S), supervisor with regime management, Rust FFI acceleration, and review-only reactor-semantic decoders are guarded by local and CI verification gates. Public artifact availability is reported by the PyPI badge and immutable exact-tag release receipts; a source-tree version alone is not publication evidence. Public capability counts are generated from the manifest below rather than maintained by hand. The public Python API — the symbols exported from `scpn_phase_orchestrator.__all__` — is covered by semantic-versioning guarantees; backward-incompatible changes to it will bump the major version. Internal modules outside that surface may still evolve.

**Version:** 1.4.3
**Status:** release candidate; public availability requires the green exact-tag workflows, PyPI publication, and an immutable release receipt; public API under semantic versioning; public inventory is generated below.
...
[![codecov](https://codecov.io/gh/anulum/scpn-phase-orchestrator/graph/badge.svg?branch=main)](https://app.codecov.io/gh/anulum/scpn-phase-orchestrator)
...
[![GitHub Sponsors](https://img.shields.io/github/sponsors/anulum?logo=github&label=Sponsor)](https://github.com/sponsors/anulum)
...
For the exact current meaning of reactor phase, mode, regime, event, and clock
surfaces across SPO, SCPN-FUSION-CORE, SCPN-MIF-CORE, and SCPN-CONTROL, see the
[reactor signal occurrence ledger](docs/reference/reactor_signal_occurrence_ledger.md).
Its 39 source groups are revision- and blob-digest-pinned; occurrence never
...
review. It binds all four unselected spherical-tokamak observability candidates
and twelve missing producer-evidence obligations, including phenomenon,
reproducible source ingestion, calibration, geometry/frame, modal operator,
clock, uncertainty, validity, quality, observability, and independent-
...
Version `1.0.0` is the first **stable** release: the public Python API — the
symbols exported from `scpn_phase_orchestrator.__all__` — is now covered by
semantic-versioning guarantees. It consolidates the operator, evaluation, and
assurance surfaces built up to the `0.12.0` baseline — including the **honest
early-warning auditor** (`scpn_phase_orchestrator.evaluation` and
`spo audit-detector`): score any detector's event-vs-null skill at a matched
false-alarm rate, with a label-permutation p-value and a hash-sealed record — the
productised form of the matched-false-alarm methodology, judging the SCPN suite, an
AR(1)/Kendall-τ baseline, or a black-box classifier on identical footing. The
release keeps the downstream accelerator chain in a single documented review
boundary:
