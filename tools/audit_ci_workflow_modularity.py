...
# SCPN Phase Orchestrator — CI Workflow Modularity Audit
...
_CALLER_KEYS = {"uses", "needs", "permissions", "with", "secrets"}
    ...
    for root_name in ("tests", "tools", "scripts"):
        for path in (REPOSITORY_ROOT / root_name).rglob("*.py"):
            ...
            if literal in path.read_text(encoding="utf-8"):
                errors.append(
                    f"{path}: use the distributed CI inventory instead of {literal}"
                )


def _check_category_graph(policy: WorkflowPolicy, errors: list[str]) -> None:
    """Require an acyclic caller graph whose edges target declared categories."""
    graph = {
        category["id"]: category["caller_needs"] for category in policy["categories"]
    }
    declared = set(graph)
    for category_id, dependencies in graph.items():
        unknown = set(dependencies) - declared
        if unknown:
            errors.append(
                f"CI category {category_id} depends on unknown categories "
                f"{sorted(unknown)}"
            )
        if category_id in dependencies:
            errors.append(f"CI category {category_id} depends on itself")
    ...
    def visit(category_id: str) -> None:
        if category_id in visiting:
            errors.append(f"CI category dependency cycle includes {category_id}")
            return
        if category_id in visited:
            return
        visiting.add(category_id)
        for dependency in graph.get(category_id, []):
            if dependency in graph:
                visit(dependency)
        visiting.remove(category_id)
        visited.add(category_id)

    for category_id in graph:
        visit(category_id)
...
def audit_ci_workflow_modularity(policy: WorkflowPolicy | None = None) -> list[str]:
    ...
    _check_category_graph(resolved, errors)

    workflow_root = REPOSITORY_ROOT / ".github/workflows"
    expected_category_paths = {
        REPOSITORY_ROOT / category["workflow"] for category in categories
    }
    if set(workflow_root.glob("ci-*.yml")) != expected_category_paths:
        errors.append(
            "physical CI category workflows do not match the versioned policy"
        )
    ...
    if set(coordinator) != _COORDINATOR_KEYS:
        errors.append("CI coordinator contains unsupported top-level keys")
    ...
    if set(coordinator_jobs) != expected_coordinator_jobs:
        errors.append(
            "CI coordinator jobs do not match category calls plus the required gate"
        )
    ...
    for category in categories:
        ...
        if lines > limits["reusable_max_lines"]:
            errors.append(
                f"{path}: {lines} lines exceed {limits['reusable_max_lines']}"
            )
        ...
        if workflow.get("permissions") != _READ_ONLY_PERMISSIONS:
            errors.append(
                f"{path}: reusable category must retain read-only contents permission"
            )
        jobs = workflow.get("jobs", {})
        if not isinstance(jobs, dict):
            errors.append(f"{path}: jobs must be a mapping")
        ...
        for job_id, job in jobs.items():
            if job_id in owned_jobs:
                errors.append(
                    f"CI job {job_id} is duplicated in {owned_jobs[job_id]} "
                    f"and {category_id}"
                )
            ...
            if external:
                errors.append(
                    f"{path}: job {job_id} has cross-category needs {sorted(external)}"
                )
            if job_id in resolved["optional_jobs"] and not (
                "if" in job or job.get("continue-on-error") == "true"
            ):
                errors.append(
                    f"{path}: optional job {job_id} lacks an explicit "
                    "condition or waiver"
                )
        ...
        if not isinstance(caller, dict):
            ...
            if not set(caller).issubset(_CALLER_KEYS):
                errors.append(
                    f"CI coordinator call {category_id} contains executable fields"
                )
            if caller.get("uses") != f"./{category['workflow']}":
                errors.append(
                    f"CI coordinator call {category_id} targets the wrong workflow"
                )
            if _needs(caller) != category["caller_needs"]:
                errors.append(
                    f"CI coordinator call {category_id} has incorrect dependencies"
                )
    ...
    if set(owned_jobs) != set(resolved["job_order"]) or set(declared_jobs) != set(
        ...
        errors.append(
            "distributed CI job inventory is incomplete or contains undeclared jobs"
        )
    ...
    if not isinstance(gate, dict):
        ...
        if _needs(gate) != category_ids:
            errors.append(
                "required CI gate does not aggregate every category exactly once"
            )
        if gate.get("if") != "always()":
            errors.append("required CI gate must run with if: always()")
        gate_text = _job_text(coordinator_path, gate_id)
        if (
            "CATEGORY_RESULTS: ${{ toJSON(needs) }}" not in gate_text
            or "run: python tools/check_ci_category_results.py" not in gate_text
        ):
            errors.append(
                "required CI gate must fail closed over every category result"
            )
...
def main() -> int:
    ...
    print(
        ...
        f"{len(policy['job_order'])} exclusively owned jobs, "
        f"stable {policy['required_gate']}"
    )
    return 0
