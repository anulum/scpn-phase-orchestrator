...
# SCPN Phase Orchestrator — CI Workflow Inventory
...
def ci_workflow_paths(policy: WorkflowPolicy | None = None) -> tuple[Path, ...]:
    ...
    paths.extend(
        REPOSITORY_ROOT / category["workflow"] for category in resolved["categories"]
    )
    return tuple(paths)
...
def read_ci_workflow_source() -> str:
    ...
    for path in ci_workflow_paths(policy)[1:]:
        for job_id, block in _job_blocks(path.read_text(encoding="utf-8")).items():
            if job_id in blocks:
                raise ValueError(
                    f"CI job appears in multiple reusable workflows: {job_id}"
                )
            blocks[job_id] = block
