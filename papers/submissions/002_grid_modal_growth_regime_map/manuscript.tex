...
\thanks{M.~\v{S}otek is with the Anulum Institute, Marbach,
Switzerland (e-mail: protoscience@anulum.li; ORCID
0009-0009-3560-0851).}}
\markboth{Submitted to IEEE Transactions on Power Systems, August
2026}{\v{S}otek: A Domain-Specific Modal-Growth Detector and an
Eigenvalue Regime Map}
...
validated on the held-out half. Stress tests then bound the result.
Run as a causal stream, the certified per-window operating point is
not deployable, and an honest recalibration with a persistence
debounce and an exponential-fit-quality gate recovers 11 of 45
held-out transitions at a matched 10\,\% stream false alarm. On two
cross-dataset corpora (three real ISO-NE PMU captures; the 13
synthetic WECC 240-bus cases of the 2021 IEEE-NASPI contest), the
certified numeric threshold does not transfer in either direction,
while the frozen shape with per-system matched-false-alarm calibration
remains usable. The form does not transfer to scalp EEG or to
few-timepoint molecular indices. Finally, against ANDES small-signal
eigenvalues of the IEEE-39 and Kundur systems, the analytic Jacobians
of the fold, pitchfork and Hopf normal forms, and the mean-field
eigenvalues of unimodal and bimodal Kuramoto models, the estimated
quantity is confirmed to be the eigenvalue's real part, yielding a
regime map: the envelope-growth family sizes the eigenvalue on
...
\begin{IEEEkeywords}
Power system stability, small-signal stability, wide-area monitoring,
phasor measurement units, early warning, oscillation detection,
eigenvalues, false-alarm rate.
\end{IEEEkeywords}
...
\begin{tabular}{lcc}
\toprule
Variant & Dev.\ led / 45, $p$ & Held-out led / 45, $p$ \\
\midrule
...
closed-form: sweeping the control parameter quasi-statically through
the stochastic normal form, the autocorrelation channel recovers the
Jacobian eigenvalue $\lambda$ ($-2\sqrt{\mu}$ fold, $\mu$ pitchfork)
in rank and magnitude (Spearman and Pearson $\rho = 0.98$/0.96),
because the lag-one autocorrelation of a linear-response process is
$\exp(\lambda\,\Delta t)$; the variance channel rises in step
($\rho = 0.98$). On the Hopf normal form (oscillatory, eigenvalue
$\alpha \pm i\omega$) the roles reverse: the envelope-growth family
recovers $\alpha$ in rank and magnitude ($\rho = 0.97$, mean
$|\sigma - \alpha| = 0.04$, frequency-invariant across 0.2--1.2\,Hz),
while the autocorrelation family tracks $\alpha$ only in rank (its
lag-one autocorrelation is pinned near $\cos(\omega\,\Delta t)$). The
envelope recovery degrades with ringdown SNR ($\rho$ 0.97 $\to$ 0.62),
a physical floor. Records:
...
signed mean-field component $\mathrm{Re}(Z)$ (magnitude slope 1.19
$\approx$ 1); the folded amplitude $|Z|$ ranks it ($\rho = 0.98$) but
cannot size it (slope 2.66). For a symmetric bimodal frequency law
(Martens et al.\ 2009) the sub-critical eigenvalue is complex and the
roles reverse exactly as the Hopf bridge predicts: the envelope-growth
family on the sub-population order parameter $|Z_+|$ recovers
$\mathrm{Re}(\lambda)$ in magnitude ($\rho = 0.99$, slope 0.93;
$N = 4000$ ringdown), the autocorrelation family is confounded by the
oscillation $\Omega$ (slope 15.8), the global $|Z|$ is a standing wave
that cannot be fit, and the measured $\Omega$ matches the analytic
value to $\sim$7\,\%. Limits: finite-$N$ ($O(1/\sqrt{N})$),
quasi-static or noiseless ringdown sweeps, coupling below $K_c$.
Records:
...
python -m bench.early_warning_leadtime_isone \
    --data-dir ISONE --output OUT
python -m bench.early_warning_leadtime_wecc \
    --data-dir WECC --output OUT
...
AI-assisted technologies (Anthropic Claude, a large-language-model
system) were used to assist with software development, analysis
tooling and manuscript drafting; this disclosure follows the IEEE
guidelines for AI-generated text. The author specified, verified and
approved all methods, analyses and claims, and takes full
responsibility for the content of this work. Competing interests: the
author develops the open-source SCPN Phase Orchestrator software used
in this study, which is also offered under a commercial licence. This
