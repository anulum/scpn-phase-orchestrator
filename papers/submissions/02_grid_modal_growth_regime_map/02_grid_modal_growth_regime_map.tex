...
oscillations (UTK test-cases library), the frozen PSML per-window
threshold ($1.3203\,\sigma/\mathrm{s}$, two-second windows) fires on
$\sim$32\,\% of pre-onset ambient windows against the certified
9.09\,\% --- too permissive there. On the 13 synthetic WECC 240-bus
forced-oscillation cases of the 2021 IEEE-NASPI Oscillation Source
Location contest (bus-voltage observable, the certified observable
family), the same threshold crosses nothing --- 0 of 611 pooled ambient
windows and 0 transition windows --- too conservative there. A
threshold certified on 238\,Hz fault-transient envelopes behaved as a
corpus-specific quantity on both corpora tested; the deployment
procedure therefore calibrates per system on the target system's own
ambient data and re-seals.
...
permutation $p = 0.332$ at $n = 3$: a case study, no significance
claim. On WECC, the corpus structure dominates: with the onset search
pinned at the exact forcing start ($t = 30$\,s, known by simulation
design), 9 of 13 cases are already past three times the ambient in-band
envelope at the forcing start --- an effectively instantaneous onset
with no early-warning window for any detector at these 90\,s record
lengths --- and of the 4 cases with a resolvable window the locally
calibrated shape leads 2 ($+5.4$\,s, $+4.1$\,s; $p = 0.41$, not
significant). A secondary branch of the sealed record reports
post-onset detection descriptively (12 of 13 cases, median latency
$\sim$1\,s) together with an explicit per-case chance bound showing the
detection count is chance-compatible at this false-alarm budget on
60\,s regions; no significance is claimed for it. Records:
...
envelope --- with the identical focal aggregation, recency weighting
and $R^2$ gate --- leads 0 of 6 preictal segments ($p = 1.00$) in both
aggregations, below the rank trend's 1 of 6; the fit-quality gate,
built to reject non-exponential fault transients, rejects the preictal
signal too. A robustness grid over window length, recency weighting and
gate leaves the score at 0 of 6 at every setting, so this is a model
mismatch, not a parameter artefact: the grid statistic presupposes a
linear instability's eigenvalue, and the preictal band-power rise, if
present, is a slow non-exponential drift. (A shorter 600\,s horizon
surfaces a spectral rank-trend lead of 3 of 6, $p = 0.04$; with one
subject and a single-horizon forking path it is disclosed in the sealed
record as exploratory and not claimed.) Record:
...
four points bulk), the discriminating machinery has no purchase: every
monotone rise fits an exponential well enough ($R^2 = 0.67$--$0.86$) to
pass the gate, and the growth rate re-orders the lineages exactly as
the existing linear slope does. The transfer collapses to the detector
DNB already uses, so the form cannot even be posed. Record:
...
score a ringdown from a small disturbance at each point. The detector's
...
aggregation --- the PSML winner --- does not transfer here
($\rho = -0.59$ and $-0.27$): on a slow coherent inter-area mode the
per-bus maximum locks onto local excursions. The growth-rate quantity
generalises; the best aggregation is regime-dependent (focal for fast
localised oscillations, coherent for slow inter-area modes). Limits: a
simulated ground truth, stable operating points only (a damping-ranking
test, not a stable-versus-unstable classification), fourteen points per
system. Record:
...
The grid result shows that a detector reading a deterministic,
directly-measurable instability quantity can clear an operational
evaluation bar that generic early-warning detectors do not clear ---
and the counterpoint shows domain specificity alone is not sufficient:
the same idea on a domain without such a signature stays at chance. The
stress tests bound the claim in deployment-relevant ways: the streaming
recalibration replaces the flattering per-window figure with an honest
...
form applies at all. The external validations tie the detector to a
named first-principles quantity --- the eigenvalue's real part ---
confirmed against a simulator's small-signal analysis, three analytic
normal forms, and two mean-field Kuramoto reductions, yielding one
operational rule: use the envelope-growth family on oscillatory modes
and the autocorrelation family on non-oscillatory ones, and on
collective coordinates read the signed mean field rather than the
folded order-parameter amplitude.
