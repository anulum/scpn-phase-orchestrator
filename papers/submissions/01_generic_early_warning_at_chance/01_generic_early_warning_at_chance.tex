...
dynamical-network-biomarker (DNB) indices for cell-fate and injury
transitions --- reaches the same conclusion under modality-appropriate
nulls: the single-cell transition index of Mojtahedi et al.\ 2016 clears
a matched operating point on 1 of 3 lineages ($p = 0.27$), and the bulk
GSE2565 index does not beat a surrogate null that is granted the same
module-selection freedom as the analysis ($p = 0.39$). Because the
corpora are small (3--12 transitions per domain), these null results
bound the demonstrated skill rather than proving early warning
impossible; what they do establish is that the operational bar is
materially stricter than the retrospective per-record test on which most
published EWS evidence rests. Every alarm and every non-detection is
sealed in content-addressed, byte-reproducible evidence records. A
companion manuscript shows that a domain-specific detector can clear the
identical bar where the domain carries a deterministic instability
signature.
\end{abstract}
...
The result is not ``nothing works''. It is: at a matched operating
point and a permutation significance test, nothing reaches significance
at these corpus sizes --- neither the suite nor the canonical
literature detector, in any of the four domains. Two readings are
compatible with this and both should be held. First, the corpora are
small (Section~\ref{sec:limitations}): a modestly skilled detector
could remain non-significant at $n = 6$--$12$, so the null bounds the
demonstrated skill rather than proving absence of skill. Second, and
independently of power: the literature's positive results come from a
retrospective per-record significance test, and this operational
protocol is materially stricter than it --- the same detectors, the
same data, and the bar moves the conclusion. The one place a detector
comes close --- the AR(1) trend on scalp EEG --- is consistent with the
seizure-prediction literature's use of autocorrelation features and
warrants a larger, higher-power EEG corpus.
...
published EWS results have cleared only the retrospective one.
