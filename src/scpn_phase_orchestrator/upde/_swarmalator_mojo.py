...
_EXE_PATH = (
    Path(__file__).resolve().parents[3] / "mojo" / "swarmalator_mojo"
)
...
def swarmalator_step_mojo(
    ...
    exe = _ensure_exe()
    tokens: list[str] = [
        "STEP", str(int(n)), str(int(dim)),
        repr(float(a)), repr(float(b)), repr(float(j)),
        repr(float(k)), repr(float(dt)),
    ]
    ...
    if proc.returncode != 0:
        raise ValueError(
            f"Mojo swarmalator exit {proc.returncode}: "
            f"{proc.stderr.strip()}"
        )
    ...
    if len(lines) != expected:
        raise ValueError(
            f"Mojo STEP returned {len(lines)} lines, expected {expected}"
        )
    new_pos = np.array(
        [float(x) for x in lines[: n * dim]], dtype=np.float64,
    )
    new_phases = np.array(
        [float(x) for x in lines[n * dim :]], dtype=np.float64,
    )
