...
# SCPN Phase Orchestrator — UPDE Julia runtime probing

"""Shared UPDE Julia runtime availability probe."""
...
def require_juliacall_main() -> ModuleType:
    ...
    -------
    ModuleType
        Imported ``juliacall`` module with its injected ``Main`` symbol.

    Raises
    ------
    ImportError
        Raised when ``juliacall`` is missing, or when it imported without the
        runtime ``Main`` symbol required by the direct Julia bridges.
    """
