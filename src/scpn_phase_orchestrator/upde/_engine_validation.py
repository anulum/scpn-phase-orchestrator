...
# SCPN Phase Orchestrator — UPDE engine validation contracts

"""Shared input and output contracts for stateless UPDE dispatch."""
...
def _as_real_finite_array(value: Any, *, name: str) -> FloatArray:
    ...
    if isinstance(value, np.ndarray):
        array = value
        has_boolean = array.dtype == np.bool_ or (
            array.dtype == np.object_ and contains_boolean_alias(array)
        )
    else:
        has_boolean = contains_boolean_alias(value)
...
def _as_vector(value: Any, *, name: str) -> FloatArray:
    """Return ``value`` as a validated non-empty finite vector, else raise."""
...
def _as_finite_real(value: Any, *, name: str, positive: bool = False) -> float:
    """Return ``value`` as a finite real float, else raise."""
...
def _as_positive_int(value: Any, *, name: str) -> int:
    """Return ``value`` as a positive integer, else raise."""
    if contains_boolean_alias(value):
        raise TypeError(f"{name} must be an integer, not boolean")
    if not isinstance(value, Integral):
        raise TypeError(f"{name} must be an integer")
...
def _validate_common_inputs(
    ...
    """Validate arguments shared by fixed-frequency and schedule dispatch."""
    n = int(phases.size)
    k = _as_square_flat(knm, name="knm", n=n)
    if np.any(np.diag(k.reshape((n, n))) != 0.0):
        raise ValueError("knm self-coupling diagonal must be zero")
    a = _as_square_flat(alpha, name="alpha", n=n)
    return (
        k,
        a,
...
def validate_upde_backend_inputs(
    ...
    """Normalise and validate stateless UPDE call arguments."""
    ...
    n_steps_i = _as_non_negative_int(n_steps, name="n_steps")
    k, a, zeta_f, psi_f, dt_f, method_s, substeps, atol_f, rtol_f = (
...
def validate_upde_schedule_backend_inputs(
    ...
    """Normalise stateless UPDE arguments with per-step frequencies."""
    p = _as_vector(phases, name="phases").copy()
