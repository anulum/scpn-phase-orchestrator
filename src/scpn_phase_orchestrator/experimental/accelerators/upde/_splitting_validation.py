...
# SCPN Phase Orchestrator - splitting backend validation contracts

"""Shared validation for direct Strang-splitting accelerator backends."""
...
__all__ = ["validate_splitting_inputs", "validate_splitting_output"]

FloatArray: TypeAlias = NDArray[np.float64]
ValidatedSplittingInputs: TypeAlias = tuple[
    FloatArray,
    FloatArray,
    FloatArray,
    FloatArray,
...
def validate_splitting_output(value: Any, *, n: int) -> FloatArray:
    ...
    if np.any(out < 0.0) or np.any(out >= TWO_PI):
        raise ValueError("splitting backend output phases must be in [0, 2*pi)")
