...
- Digital-twin binding standard: version `binding_spec.yaml` as an open bidirectional live-sync contract for simulators, services, and hardware twins.
  ...
  - JSONL file replay adapter is in place for deterministic offline adapter smoke tests, separating accepted validations from malformed JSON, invalid envelope shapes, and contract-validation rejections. gRPC/Kafka/hardware adapters remain open.
