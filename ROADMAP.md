...
- Digital-twin binding standard: version `binding_spec.yaml` as an open bidirectional live-sync contract for simulators, services, and hardware twins.
  ...
  - Transport-neutral payload validation is in place: digital-twin sync envelopes validate contract hashes, declared capabilities, directions, sequence numbers, and non-empty payloads before a REST/gRPC/Kafka/file/hardware adapter hands data to runtime code. Concrete live adapters remain open.
  - JSONL file replay adapter is in place for deterministic offline adapter smoke tests, separating accepted validations from malformed JSON, invalid envelope shapes, and contract-validation rejections. REST/gRPC/Kafka/hardware adapters remain open.
  ...
  - Adapter manifest compatibility checks are in place for concrete transport review: manifests declare transport type, sync capabilities, replay support, and auth posture before REST/gRPC/Kafka/hardware adapter code is enabled.
