...
def run_sparse_bench(n=1000, density=0.01, n_steps=100):
    ...
    for _ in range(n_steps):
        phases = engine.step(phases, omegas, row_ptr, col_indices, knm_values, 0.0, 0.0, alpha_values)
    elapsed = time.perf_counter() - t0
    
    R, _ = compute_order_parameter(phases)
    print(f"Sparse Engine (N={n}, density={density}): {n_steps/elapsed:.1f} steps/s, R={R:.4f}")
