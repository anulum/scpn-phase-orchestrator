# SPDX-License-Identifier: AGPL-3.0-or-later
# Commercial license available
# © Concepts 1996–2026 Miroslav Šotek. All rights reserved.
# © Code 2020–2026 Miroslav Šotek. All rights reserved.
# ORCID: 0009-0009-3560-0851
# Contact: www.anulum.li | protoscience@anulum.li
# SCPN Phase Orchestrator — Envelope multi-backend benchmark

"""Per-backend wall-clock benchmark for
``upde.envelope.extract_envelope`` (1-D path)."""

from __future__ import annotations

import argparse
import json
import time
from pathlib import Path

import numpy as np

from scpn_phase_orchestrator.upde import envelope as e_mod
from scpn_phase_orchestrator.upde.envelope import (
    ACTIVE_BACKEND,
    AVAILABLE_BACKENDS,
    extract_envelope,
)


def _bench(backend: str, amps, window: int, calls: int) -> float:
    saved = e_mod.ACTIVE_BACKEND
    try:
        e_mod.ACTIVE_BACKEND = backend
        extract_envelope(amps, window)
        t0 = time.perf_counter()
        for _ in range(calls):
            extract_envelope(amps, window)
        return time.perf_counter() - t0
    finally:
        e_mod.ACTIVE_BACKEND = saved


def bench_at(n: int, window: int, calls: int) -> dict:
    rng = np.random.default_rng(42)
    amps = np.abs(rng.normal(1.0, 0.3, n))
    row: dict = {
        "N": n, "window": window, "calls": calls,
        "available": AVAILABLE_BACKENDS,
    }
    for backend in AVAILABLE_BACKENDS:
        t = _bench(backend, amps, window, calls)
        row[f"{backend}_ms_per_call"] = (t / calls) * 1000.0
    return row


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=None)
    parser.add_argument("--sizes", type=int, nargs="+", default=[500, 5000, 50000])
    parser.add_argument("--window", type=int, default=10)
    parser.add_argument("--calls", type=int, default=20)
    args = parser.parse_args()

    print(f"Active: {ACTIVE_BACKEND}  Available: {AVAILABLE_BACKENDS}\n")
    header = f"{'N':>6} {'win':>5} {'calls':>6}"
    for b in AVAILABLE_BACKENDS:
        header += f" {b + '_ms':>12}"
    print(header)
    print("-" * len(header))
    results: list[dict] = []
    for n in args.sizes:
        row = bench_at(n, args.window, args.calls)
        results.append(row)
        line = f"{n:>6} {args.window:>5} {args.calls:>6}"
        for b in AVAILABLE_BACKENDS:
            line += f" {row[f'{b}_ms_per_call']:>12.4f}"
        print(line)
    if args.output:
        args.output.write_text(
            json.dumps({"results": results}, indent=2), encoding="utf-8"
        )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
