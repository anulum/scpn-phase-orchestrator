...
import numpy as np
from spo_kernel import PyActiveInferenceAgent, PyUPDEStepper
...
def run_bench():
    ...
    for _ in range(100):
        phases = engine_sparse.step(
            phases, omegas, row_ptr, col_indices, knm_values, 0.0, 0.0, alpha_values
        )
    t1 = time.perf_counter()
    us = (t1 - t0) / 100 * 1e6
    print(f"{'Sparse UPDE (Rust)':<40} | N={n_sparse:<13} | {us:>8.2f} us/step")
    ...
    lbl = f"{'Dense Baseline (Rust)':<40} | N={n_dense:<13}"
    print(f"{lbl} | {base_latency:>8.2f} us/step")
    ...
    lbl = f"{'Dense + Plasticity (Rust)':<40} | N={n_dense:<13}"
    print(f"{lbl} | {plast_latency:>8.2f} us/step")
    overhead = plast_latency - base_latency
    print(f"{'  Plasticity Overhead':<40} | {'':15} | {overhead:>8.2f} us/step")
    ...
    t1 = time.perf_counter()
    us = (t1 - t0) / 1000 * 1e6
    print(f"{'Active Inference (Rust)':<40} | Hidden=16 {'':5} | {us:>8.2f} us/step")
    ...
    t1 = time.perf_counter()
    ms = (t1 - t0) / 10 * 1e3
    print(f"{'Gauged PGBO (Python/NumPy)':<40} | N={n_pgbo:<13} | {ms:>8.2f} ms/obs")
