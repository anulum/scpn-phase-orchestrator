...
| Formal supervisor verification | Petri-net and policy surfaces export to PRISM and TLA+; PRISM STL label export exists; deterministic formal verification packages now bind exported artefact hashes, named safety properties, and external PRISM/TLC checker command manifests without invoking tools locally; deeper external model-checker CI execution remains open |
...
| Plugin ecosystem | manifest registry foundation is implemented for entry-point discovery, capability declarations, compatibility checks, audit records, marketplace catalogue packaging, a runnable catalogue example, CLI catalogue export, Rust-facing flattened registry export, and guarded metadata-only Rust runtime handoff manifests/CLI export with grouped dispatch records, target hashes, blocked incompatible entries, mutual-exclusive Rust output modes, and loading disabled by default; deeper owned runtime loading remains open |
