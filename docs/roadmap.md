...
| Quantum-native compiler target | deterministic OpenQASM 3 compiler manifests now cover Qiskit/PennyLane handoff with Z-frequency terms, symmetrised XY coupling terms, co-simulation parity evidence, SHA-256 hashes, QPU execution disabled, and non-executing target-readiness audits for backend, credential, operator-approval, and stable-hash evidence; real QPU target execution remains open |
