...
| Item | Target outcome |
|------|----------------|
| One-click SPO Studio web UI | Streamlit operator prototype now covers domainpack load, raw-source import, binding review, beginner-mode guidance with ordered walkthrough, oscillator edit review artefacts, layer/channel canvas review artefacts, canvas layout manifests, canvas topology patch artefacts, validated binding rewrite candidates, signed hash-checked binding apply with backups, live `R`/`Psi`/`K` metrics, replay tuning, hierarchy monitor, connector ownership plans, dry-run records, and owned no-I/O runtime boundary validation for REST/gRPC/Kafka/hardware, deployment readiness/package/materialisation manifests, hardware target and verified-evidence package manifests, recovery reports, and Docker/WASM/project export manifests; it is not yet a good standalone product, because deployed live service processes, browser polish, and real FPGA/neuromorphic evidence remain open |
