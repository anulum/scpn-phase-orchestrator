...
| Coverage debt closure | historical fixes added behaviour coverage for five modules, local coverage guard passes, focused Studio/CLI/supervisor behaviour tests cover residual service-manifest, auto-coupling CLI, formal-readiness, policy-branching, and hierarchy-ingestion paths, and focused quantum/causal/meta/autotune/reporting/QPU tests cover residual adapter validation, deterministic manifests, malformed-input rejection, and artefact parsing paths | reduce residual source coverage debt versus the historical CI baseline, then refresh the internal coverage inventory from current CI artefacts |
