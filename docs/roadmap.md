...
| Formal supervisor verification | Petri-net and policy surfaces export to PRISM and TLA+; PRISM STL label export exists; benchmark-gated deterministic formal verification packages now bind exported artefact hashes, named safety properties, disabled checker execution, and external PRISM/TLC checker command manifests without invoking tools locally; `spo formal-export --export package` exposes the same no-execution package manifest; deeper external model-checker CI execution remains open |
