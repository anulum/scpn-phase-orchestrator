...
| Item | Target outcome |
|------|----------------|
| One-click SPO Studio web UI | Streamlit operator prototype now covers domainpack load, raw-source import, binding review, beginner-mode guidance, oscillator edit review artefacts, layer/channel canvas review artefacts, canvas layout manifests, canvas topology patch artefacts, live `R`/`Psi`/`K` metrics, replay tuning, hierarchy monitor, connector ownership plans, deployment readiness/package manifests, hardware target package manifests, recovery reports, and Docker/WASM/project export manifests; it is not yet a good standalone product, because live drag/drop binding rewrite, richer guided onboarding, live connector execution, package materialisation commands, and verified FPGA packaging remain open |
...
| Guided deployment path | deployment readiness, deployment package, and hardware target manifests now carry target status, required artefacts, export hashes, commands, blocked reasons, safety gates, hardware evidence requirements, and connector handoff review; package materialisation commands and verified hardware evidence remain open |
