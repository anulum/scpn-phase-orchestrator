...
| Plugin ecosystem | manifest registry, entry-point discovery, capability declarations, compatibility checks, audit records, marketplace catalogue, Rust-facing registry export, deterministic metadata-only planning artefacts, `spo plugins plan-execution` output, approval-consumption `spo plugins request-execution PLAN_JSON APPROVAL_JSON` request artefacts, request-bound runtime invocation with pre-import plan/target/shape revalidation, stored-request hash validation and revocation checks, deterministic request storage manifests with retention and custody metadata, guarded metadata-only Rust handoff manifests, grouped dispatch records, explicit loading-disabled gates, Python-owned opt-in loading gates with manifest compatibility/package-boundary/target-callability checks, deterministic execution audit records, pre-import target-hash approval, operator identity/reference binding to `plan_hash` and `target_hash`, runtime policy envelopes that require `require_target_hash_approval` and a matching approved `target_hash` for load/execute flags, and argument-value-free execution records are implemented | harden deployment-interface persistence adapters and operator request lifecycle surfaces |
