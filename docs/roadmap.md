...
| Item | Target outcome |
|------|----------------|
| One-click SPO Studio web UI | Streamlit operator prototype now covers domainpack load, raw-source import, binding review, beginner-mode guidance, oscillator edit review artefacts, layer/channel canvas review artefacts, live `R`/`Psi`/`K` metrics, replay tuning, hierarchy monitor, deployment readiness/package manifests, recovery reports, and Docker/WASM/project export manifests; it is not yet a good standalone product, because direct drag/drop layout persistence, richer guided onboarding, live connectors, package materialisation commands, and FPGA packaging remain open |
