...
| Plugin ecosystem | manifest registry, entry-point discovery, capabilities, compatibility checks, audit records, marketplace catalogue, runnable example, CLI export, Rust-facing registry export, guarded metadata-only Rust handoff manifests, grouped dispatch records, loading-disabled gates, explicit Python-owned runtime loading with manifest compatibility, package-boundary, callable-target, and deterministic audit gates, plus opt-in runtime invocation with pre-import denial and argument-value-free execution audit records are implemented | build deeper owned runtime execution paths around approved deployment/plugin interfaces |
