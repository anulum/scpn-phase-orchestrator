...
| Cross-domain meta-transfer | replay-backed proposals, multi-audit fitting, nested audit-directory corpus loading, training summaries, deterministic JSON package export, `scpn-meta` packaging-readiness manifests, and reference-suite manifest-readiness gates are implemented; larger real audit-history corpora and actual optional distribution packaging remain open |
