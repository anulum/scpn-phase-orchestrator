...
| Formal supervisor verification | Petri-net and policy surfaces export to PRISM and TLA+; PRISM STL label export exists; benchmark-gated deterministic formal verification packages now bind exported artefact hashes, named safety properties, disabled checker execution, external PRISM/TLC checker command manifests, and checker-readiness records without invoking tools locally; `spo formal-export --export package` exposes the same no-execution package manifest; non-executing external-checker readiness audits now report PRISM/TLC executable availability, resolved paths, ready/missing status, and disabled execution; deeper external model-checker CI execution remains open |
