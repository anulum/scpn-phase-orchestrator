...
| Item | Target outcome |
|------|----------------|
| One-click SPO Studio web UI | Streamlit operator prototype now covers domainpack load, raw-source import, binding review, oscillator edit review artefacts, live `R`/`Psi`/`K` metrics, replay tuning, hierarchy monitor, and Docker/WASM/project export manifests; it is not yet a good standalone product, because true drag/drop canvas editing, guided beginner mode, live connectors, deployment packaging, polished recovery, and FPGA packaging remain open |
