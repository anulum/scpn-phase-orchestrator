...
| Plugin ecosystem | manifest registry, entry-point discovery, capabilities, compatibility checks, audit records, marketplace catalogue, runnable example, CLI export, Rust-facing registry export, deterministic metadata-only runtime plan output, `spo plugins plan-execution` operator JSON, guarded metadata-only Rust handoff manifests, grouped dispatch records, loading-disabled gates, explicit Python-owned runtime loading with manifest compatibility, package-boundary, callable-target, and deterministic audit gates, opt-in runtime invocation with pre-import denial and argument-value-free execution audit records, and pre-import target-hash approval for reviewed deployment paths are implemented | build the next owned runtime execution pathway only after explicit operator sign-off and deployment-interface policy are defined |
...
| Plugin ecosystem | manifest registry foundation is implemented for entry-point discovery, capability declarations, compatibility checks, audit records, marketplace catalogue packaging, a runnable catalogue example, CLI catalogue export, Rust-facing flattened registry export, benchmark-gated guarded metadata-only Rust runtime handoff manifests/CLI export with grouped dispatch records, target hashes, blocked incompatible entries, mutual-exclusive Rust output modes, deterministic hashes, loading disabled by default, Python-owned opt-in runtime loading for approved callable capability targets, opt-in runtime execution with deterministic execution audits, pre-import target-hash approval for production deployment gates, a non-loading planning artefact layer with immutable invocation-shape records (argument values withheld), and `spo plugins plan-execution` operator JSON output; deeper owned runtime execution paths remain open |
