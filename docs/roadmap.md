...
| STL runtime verification | robustness monitor foundation, policy YAML `stl_monitors` integration, PRISM export linkage, builtin monitoring automata synthesis, non-actuating controller-candidate synthesis, policy-gated candidate projection, and offline closed-loop synthesis plans are implemented; runtime integration through the full safety/actuation stack remains open |
