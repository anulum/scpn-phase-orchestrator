...
| Hierarchical orchestration | reduced-summary parent orchestration, bounded escalation audit records, deterministic edge/cloud sync envelopes, stateful non-socket sync-batch runtime, power-grid/cardiac replay demos, and offline gossip consensus replay are implemented; live transport adapters and broader multi-domain demos remain open |
