...
| Auto-binding prototype | deterministic proposal builders now infer reviewable `binding_spec.yaml` records from time-series CSV, event-log JSON, and graph JSON with confidence factors, validation diagnostics, time-column sample-rate inference, and sparse-derivative/correlation-graph/clustering evidence; `spo auto-bind` exports review-only binding YAML or audit JSON from local raw sources without writing files or enabling actuation; deeper graph-learning inference remains experimental |
...
|------|------------------|
| Auto-discovery / data-driven binding | raw CSV, sensor logs, and event streams produce reviewable oscillator, channel, initial `K`, and binding proposals via SINDy, coupling estimation, clustering, and validation diagnostics; zero-config CSV sample-rate inference, sparse-derivative evidence, phase-aware Kuramoto SINDy evidence, correlation graph evidence, and transfer-entropy coupling audit export are in place; benchmarked multi-library SINDy selection, learned graph inference, extractor-parameter proposals, and initial `K` binding remain open |
| Bayesian UPDE uncertainty | sampled `omega` and `K_nm` distributions propagate through the existing UPDE kernel and report posterior-predictive `R ± sigma`, credible intervals, sampled final phases, and JSON-safe audit diagnostics; posterior fitting and benchmarked NumPyro/BlackJAX samplers remain open |
...
| Value-alignment supervisor guard | guard foundation, binding-spec templates, counterfactual violation/score reporting, and all current domainpack templates are implemented, including cardiac rhythm, power grid, autonomous vehicle, satellite, power-safety N-channel, network-security, financial-markets, chemical-reactor, manufacturing SPC, robotic CPG, swarm-robotics, traffic-flow, plasma-control, fusion-equilibrium, neuroscience EEG, brain-connectome, sleep-architecture, circadian-biology, epidemic SIR, agent-coordination, quantum-simulation, identity-coherence, PLL-clock, digital-twin N-channel, edge-consensus N-channel, firefly-swarm, QueueWaves, rotating-machinery, laser-array, bio-stub, vortex-shedding, musical-acoustics, gene-oscillator, geometry-walk, metaphysics-demo, and minimal-domain; production calibration against real deployment or replay evidence remains open |
