...
| Causal intervention engine | counterfactual rollout foundation, live causal-graph learning, and cardiac/power-grid/traffic-flow attribution demos are implemented; larger causal-model learners and additional attribution demos remain open |
...
| Hierarchical orchestration | reduced-summary parent orchestration, bounded escalation audit records, deterministic edge/cloud sync envelopes, power-grid/cardiac replay demos, and offline gossip consensus replay are implemented; live transport and broader multi-domain demos remain open |
...
| Sheaf-cohomology control | supervisor foundation, edge-consensus and power-grid heterogeneous-domain demos, and obstruction-summary triage are implemented; additional heterogeneous demos and deeper obstruction hardening remain open |
...
| Morphogenetic field topology | field foundation, dependency-free snapshot visualisation, passive SVG rendering, plus swarm-robotics split-flock, power-grid stressed-replay, traffic-flow corridor-spillback, and plasma-control edge-localised demos are implemented; additional domainpack demos and richer Studio UI rendering remain open |
| Integrated-information monitor | monitor foundation, audit-report summary integration, and deterministic approximation benchmark cases are implemented; broader benchmark corpus remains open |
