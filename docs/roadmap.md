...
| Coverage debt closure | historical fixes added behaviour coverage for five modules, local coverage guard passes, and focused Studio/CLI/supervisor behaviour tests now cover residual service-manifest, auto-coupling CLI, formal-readiness, policy-branching, and hierarchy-ingestion paths | reduce residual source coverage debt versus the historical CI baseline, then refresh the internal coverage inventory from current CI artefacts |
