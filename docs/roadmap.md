...
| Plugin ecosystem | manifest registry foundation is implemented for entry-point discovery, capability declarations, compatibility checks, audit records, marketplace catalogue packaging, a runnable catalogue example, CLI catalogue export, Rust-facing flattened registry export, and guarded metadata-only Rust runtime handoff manifests with grouped dispatch records, target hashes, blocked incompatible entries, and loading disabled by default; deeper owned runtime loading remains open |
