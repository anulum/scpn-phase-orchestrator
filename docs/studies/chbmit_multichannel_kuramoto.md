...
We compare the textbook multi-channel Kuramoto order parameter with the new
adaptive quality-weighted variant on real annotated seizures from the CHB-MIT
...
(mean detection rate 0.70, AUC 0.91), while the adaptive quality-weighted
variant lags (mean detection rate 0.43, AUC 0.80). The delta-band variants are
uninformative. This is a concrete negative result for the adaptive refinement
on this corpus: the kurtosis/SNR weighting does not improve separation and may
suppress useful channels.
...
the 4–30 Hz band. Treat the adaptive quality weights as a domain-specific
hyperparameter, not a default replacement.
