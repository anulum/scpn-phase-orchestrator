...
`PYTHONPATH=src:. .venv/bin/python benchmarks/recurrence_benchmark.py --T-list 30 100 300 --d 3 --epsilon 0.8 --calls 20`.
...
The release-grade gate is stricter than the wall-clock table:

```bash
PYTHONPATH=src:. .venv/bin/python benchmarks/recurrence_benchmark.py \
  --parity-gate --T-list 64 --d 3 --epsilon 0.8 --calls 1
```

It records all five declared backend slots and calls each resolved backend
directly, so the public fallback chain cannot hide backend drift. Available
backends must match the Python threshold reference exactly for both
``euclidean`` and ``angular`` recurrence matrices, cross-recurrence matrices,
and the identity ``cross_recurrence_matrix(a, a) == recurrence_matrix(a)``.
The gate also records the Python RQA recurrence-rate, determinism, and
laminarity contracts and fails if these leave the physical ``[0, 1]`` range.

The reference-suite key is ``recurrence_polyglot``; it is regenerated by
`PYTHONPATH=src:. .venv/bin/python benchmarks/reference_suite.py`.
