...
Public and direct delayed-Kuramoto phase, frequency, coupling, and phase-lag
arrays, shared/public backend outputs, and Julia raw returns reject
numeric-string aliases before float coercion. Mojo stdout remains an explicit
text protocol and is parsed before the shared numeric output validator runs.
