...
| Capability | SPO | Brian2 | NEST | Nengo | SciPy |
|---|:---:|:---:|:---:|:---:|:---:|
| Kuramoto / phase oscillators | **native** | manual | no | no | manual |
| Stuart-Landau amplitude model | **native** | manual | no | no | manual |
| Supervisory control loop | **yes** | no | no | no | no |
| Regime detection & hysteresis | **yes** | no | no | no | no |
| Boundary monitoring (soft/hard) | **yes** | no | no | no | no |
| Coupling adaptation (imprint) | **yes** | no | no | no | no |
| Phase-amplitude coupling (PAC) | **native** | manual | no | no | no |
| Deterministic audit replay | **yes** | no | no | no | no |
| Domain-agnostic binding spec | **yes** | no | no | no | no |
| Spiking neuron models | no | **native** | **native** | **native** | no |
| Large-scale spiking (>10^6) | no | yes | **yes** | yes | no |
| GPU acceleration | no | partial | partial | yes | no |
| Adaptive ODE solvers | RK45 | varies | no | no | **full** |
| Rust FFI kernel | **yes** | C++ | C++ | no | Fortran |
