...
|----------|------------------:|------------:|
| critical_slowing_down | 7 / 12 | 1 s |
| synchronisation | 0 / 12 | — |
| transition_entropy | 2 / 12 | 1 s |
| ensemble_weighted (fusion) | 5 / 12 | 1 s |
...
leads **fewer** instabilities than critical slowing down alone, so there is no
...
- **Matched false alarm.** Each detector's threshold is the smallest that holds the
  trial false-alarm rate at or below 10 %. The calibrated robust-z thresholds were:
  critical slowing down 10.0, rising synchronisation 3.5, transition entropy 3.75,
  fusion 10.0.
