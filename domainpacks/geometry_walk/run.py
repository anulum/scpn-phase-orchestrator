...
# SCPN Phase Orchestrator — Geometry walk example

"""Geometry walk: random -> clustering -> fragmentation -> coalescence."""
...
def main():
    ...
    zeta = spec.drivers.symbolic.get("zeta", 0.0) if spec.drivers.symbolic else 0.05
    psi_target = 0.0

    print("=== Geometry Walk: Random -> Clustering -> Fragment -> Coalescence ===\n")
    print(f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}  {'regime':>10}  phase")
    print("-" * 60)

    for step in range(STEPS):
        # Phase 1 (0-49): random walk
        # Phase 2 (50-99): clustering transition — narrow omega spread
        if step == 50:
            omegas = np.full(n_osc, 1.0) + rng.uniform(-0.1, 0.1, n_osc)

        # Phase 3 (100-149): fragmentation — phase scatter
        if step == 100:
            phases += rng.uniform(0, TWO_PI, n_osc)
            omegas = rng.uniform(0.3, 2.0, n_osc)

        # Phase 4 (150-199): coupling boost -> coalescence
        if step == 150:
            omegas = np.full(n_osc, 1.0) + rng.uniform(-0.05, 0.05, n_osc)
            coupling = CouplingState(
                knm=coupling.knm * 2.0,
