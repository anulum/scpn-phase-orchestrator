...
# SCPN Phase Orchestrator — Autonomous vehicles example

"""Autonomous vehicles: platoon formation -> cruise -> emergency brake -> recovery."""
...
def main():
    ...
    print("=== Autonomous Vehicles: Formation -> Cruise -> E-Brake -> Recovery ===\n")
    print(f"{'step':>5}  {'R_good':>6}  {'regime':>10}  phase")
    print("-" * 50)

    for step in range(STEPS):
        # Leader emergency brake at step 120: sudden phase jump
        if step == 120:
            phases[0] += np.pi
        ...
        follower_r = layer_states[1].R
        obs_values = {
            "R": mean_r,
            "gap_distance": 0.1 + 0.9 * follower_r,
            "speed_delta": 0.7 * (1.0 - follower_r),
            "brake_reaction": 0.4 * (1.0 - follower_r),
        }
