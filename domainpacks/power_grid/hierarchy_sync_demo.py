...
# SCPN Phase Orchestrator — Power-grid hierarchy sync demo
...
def power_grid_sync_states() -> dict[str, FloatArray]:
    """Return deterministic edge-region phases for hierarchy sync replay."""
    return {
...
def build_power_grid_sync_envelopes() -> tuple[HierarchySyncEnvelope, ...]:
    ...
    for sequence, (name, phases) in enumerate(states.items(), start=11):
        ...
        summary = ChildSupervisorSummary(
            ...
            metadata={"source": "pmu_replay", "oscillators": phases.size},
        )
        envelopes.append(
            build_hierarchy_sync_envelope(
                summary,
...
def run_demo() -> dict[str, object]:
    """Run parent-side ingestion for power-grid hierarchy sync envelopes."""
