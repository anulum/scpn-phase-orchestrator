...
# SCPN Phase Orchestrator — Neuroscience EEG example

"""Neuroscience EEG: seizure->alpha entrainment->meditation imprint."""
...
# Buzsaki (2006) center frequencies -> angular velocities
OMEGAS_HZ = [2, 2, 2, 6, 6, 10, 10, 10, 20, 20, 40, 40, 0.1, 0.1]
TWO_PI = 2.0 * np.pi
...
def main():
    ...
    print("=== Neuroscience EEG: Seizure -> Entrainment -> Meditation ===\n")
    hdr = f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}"
    hdr += f"  {'regime':>10}  {'violations':>4}  phase"
    print(hdr)
    print("-" * 65)

    for step in range(STEPS):
        # Phase 1 (0-74): baseline awake EEG
        # Phase 2 (75-149): seizure-like delta hypersynchrony
        if 75 <= step < 150:
            delta_ids = layer_map[0]
            phases[delta_ids] = phases[delta_ids] * 0.95 + 0.05 * phases[delta_ids[0]]

        # Phase 3 (150-224): alpha entrainment stimulus
        if 150 <= step < 225:
            zeta = 0.3
            psi_target = TWO_PI * 10 * step * spec.sample_period_s

        # Phase 4 (225-299): meditation imprint accumulation
        if step == 225:
            zeta = spec.drivers.physical.get("zeta", 0.0)
            psi_target = 0.0
        ...
        obs_values = {"R": mean_r, "broadband_sync": mean_r}
        for i, ls in enumerate(layer_states):
            obs_values[f"R_{i}"] = ls.R
        obs_values["alpha_power_ratio"] = layer_states[2].R
        obs_values["gamma_amplitude"] = layer_states[4].R
        ...
        if step % 25 == 0:
            if step < 75:
                ...
                label = "meditate"
            n_viol = (
                len(boundary_state.violations)
                if hasattr(boundary_state, "violations")
                else 0
            )
            print(
                f"{step:5d}  {r_good:.4f}  {r_bad:.4f}  "
                f"{regime_manager.current_regime.value:>10}  {n_viol:4d}  {label}"
            )
