...
# SCPN Phase Orchestrator — Cardiac rhythm example

"""Cardiac rhythm: sinus -> PVCs -> sustained VT -> drug -> pacing -> recovery."""
...
# Guyton Ch. 10 normalised angular velocities
OMEGAS = np.array(
    [
        1.17,
        0.83,
        0.58,  # sa_node (primary, secondary, subsidiary)
        0.75,
        0.67,  # atrial_conduction (AV node, His bundle)
        0.50,
        0.42,  # ventricular_depolarization
        0.33,
        0.33,
        0.33,  # repolarization (apex, base, RVED)
    ]
)
...
def main():
    ...
    print("=== Cardiac Rhythm: Sinus -> PVCs -> VT -> Drug -> Pacing -> Recovery ===\n")
    print(f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}  {'regime':>10}  phase")
    print("-" * 55)

    for step in range(STEPS):
        # Phase 1 (0-49): normal sinus rhythm
        # Phase 2 (50-99): premature ventricular contractions
        if 50 <= step < 100:
            vent_ids = layer_map[2]
            if step % 8 == 0:
                phases[vent_ids] += rng.uniform(0.5, 1.5, len(vent_ids))

        # Phase 3 (100-149): sustained ventricular tachycardia
        if 100 <= step < 150:
            vent_ids = layer_map[2]
            omegas[vent_ids] = 2.5

        # Phase 4 (150-199): drug intervention (imprint accumulates)
        if step == 150:
            omegas[layer_map[2]] = OMEGAS[layer_map[2][0] : layer_map[2][-1] + 1]
            zeta = 0.3

        # Phase 5 (200-249): overdrive pacing -> sinus recovery
        if step == 200:
            zeta = 0.5
            psi_target = TWO_PI * 1.17 * step * spec.sample_period_s
        ...
        sa_r = layer_states[0].R
        obs_values = {
            "R": mean_r,
            "heart_rate_bpm": 70 + 110 * (1.0 - sa_r),
            "qt_ms": 350 + 150 * (1.0 - sa_r),
            "st_mv": 0.1 * (1.0 - 2 * sa_r),
            "pr_ms": 120 + 80 * (1.0 - sa_r),
        }
