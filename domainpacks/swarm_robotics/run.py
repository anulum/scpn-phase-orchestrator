...
# SCPN Phase Orchestrator — Swarm robotics example

"""Swarm robotics: random -> align -> obstacle -> re-form -> leader fail."""
...
# Vicsek et al. (1995): heading angles as phase oscillators.
OMEGAS = np.array(
    [
        0.50,
        0.52,
        0.48,
        0.51,  # individual_heading
        0.20,
        0.15,  # formation_shape
        0.05,
        0.03,  # flock_direction
    ]
)
...
def main():
    ...
    print("=== Swarm Robotics: Random -> Align -> Obstacle -> Re-form ===\n")
    print(f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}  {'regime':>10}  phase")
    print("-" * 60)

    for step in range(STEPS):
        # Phase 1 (0-39): random headings, no coordination
        # Phase 2 (40-79): alignment emergence via Vicsek averaging
        if step == 40:
            zeta = 0.15

        # Phase 3 (80-119): obstacle breaks formation
        if step == 80:
            form_ids = layer_map[1]
            omegas[form_ids] *= 3.0  # formation stress
            phases[form_ids] += rng.uniform(0.5, 1.5, len(form_ids))

        # Phase 4 (120-159): re-form after obstacle
        if step == 120:
            omegas[:n_osc] = OMEGAS[:n_osc]
            zeta = 0.2

        # Phase 5 (160-199): leader failure + swarm recovery
        if step == 160:
            flock_ids = layer_map[2]
            omegas[flock_ids[0]] = 0.0  # leader drops out
            coupling_knm = coupling.knm.copy()
            coupling_knm[flock_ids[0], :] *= 0.1
            coupling_knm[:, flock_ids[0]] *= 0.1
        ...
        heading_r = layer_states[0].R
        obs_values = {
            "R": mean_r,
            "formation_error_m": 3.0 * (1.0 - layer_states[1].R),
            "min_dist_m": 0.3 + 0.7 * heading_r,
            "heading_var_rad": 1.5 * (1.0 - heading_r),
        }
