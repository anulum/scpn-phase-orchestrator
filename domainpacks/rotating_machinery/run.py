...
# SCPN Phase Orchestrator — Rotating machinery example

"""Rotating machinery: ramp -> critical speed -> bearing degradation -> flutter."""
...
OMEGAS = np.array(
    [
        1.0,
        2.0,
        3.0,  # shaft_rotation: 1X, 2X, 3X
        5.0,
        0.3,  # blade_dynamics: blade pass, flutter
        1.68,
        1.2,
        0.072,  # bearing_condition: BPFI, BPFO, FTF (SKF 6205)
        3.6,
        7.2,  # structural_resonance: mode 1, mode 2
    ]
)
...
def _apply_rotating_machinery_action(
    ...
    *,
    ...
    if action.knob == "zeta":
        return coupling, min(zeta + action.value, 2.0)
...
def main():
    ...
    print("=== Rotating Machinery: Ramp -> Critical -> Bearing -> Flutter ===\n")
    print(f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}  {'regime':>10}  phase")
    print("-" * 55)

    for step in range(STEPS):
        # Phase 1 (0-39): cold start ramp-up
        if step < 40:
            ramp = (step + 1) / 40.0
            omegas[:3] = OMEGAS[:3] * ramp

        # Phase 2 (40-79): 1st critical speed crossing
        if 40 <= step < 80:
            struct_ids = layer_map[3]
            omegas[struct_ids] = OMEGAS[struct_ids[0] : struct_ids[-1] + 1] * 0.95

        # Phase 3 (80-119): nominal speed
        if step == 80:
            omegas[:n_osc] = OMEGAS[:n_osc]

        # Phase 4 (120-159): bearing degradation onset
        if 120 <= step < 160:
            bearing_ids = layer_map[2]
            omegas[bearing_ids] += 0.005 * rng.standard_normal(len(bearing_ids))

        # Phase 5 (160-199): blade flutter event + policy response
        if step == 160:
            blade_ids = layer_map[1]
            omegas[blade_ids[1]] = 0.8
        ...
        shaft_r = layer_states[0].R
        obs_values = {
            "R": mean_r,
            "vibration_mm_s": 7.1 * (1.0 - shaft_r),
            "shaft_um": 100.0 * (1.0 - shaft_r),
            "bearing_degc": 60 + 45 * (1.0 - layer_states[2].R),
        }
        ...
        for act in actions:
            coupling, zeta = _apply_rotating_machinery_action(
                coupling,
                act,
                zeta=zeta,
                layer_map=layer_map,
