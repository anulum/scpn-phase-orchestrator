...
# SCPN Phase Orchestrator — Biology stub example

"""Bio stub: homeostasis -> circadian disruption -> immune challenge -> recovery."""
...
# Spanning biological timescales (normalised)
OMEGAS = np.array(
    [
        10.0,
        8.0,
        0.5,
        0.01,  # cellular: Ca2+, membrane, metabolic, mitotic
        1.0,
        0.8,
        0.3,
        5.0,  # tissue: ECM, vasculature, immune, neural
        1.2,
        0.25,
        0.05,
        0.04,  # organ: cardiac, respiratory, hepatic, renal
        0.001,
        0.01,
        0.5,
        0.002,  # systemic: circadian, hormonal, autonomic, immune_global
    ]
)
...
def main():
    ...
    print(
        "=== Bio Stub: Homeostasis -> Circadian Disruption -> Immune -> Recovery ===\n"
    )
    print(f"{'step':>5}  {'R_good':>6}  {'R_bad':>5}  {'regime':>10}  phase")
    print("-" * 55)

    for step in range(STEPS):
        # Phase 1 (0-49): homeostasis
        # Phase 2 (50-99): circadian disruption
        if step == 50:
            sys_ids = layer_map[3]
            phases[sys_ids] += rng.uniform(0.5, 1.5, len(sys_ids))
            omegas[sys_ids] *= 0.5

        # Phase 3 (100-149): immune challenge
        if step == 100:
            tissue_ids = layer_map[1]
            omegas[tissue_ids] *= 2.0

        # Phase 4 (150-199): recovery
        if step == 150:
        ...
        organ_r = layer_states[2].R
        obs_values = {
            "R": mean_r,
            "heart_rate_bpm": 70 + 110 * (1.0 - organ_r),
        }
        ...
        r_bad = 0.0  # no bad_layers defined
